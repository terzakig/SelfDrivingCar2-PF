...
// normal pdf
inline static double normal_pdf(double x, double mu, double stdev) {
  
  static const double inv_sqrt_2pi = 0.3989422804014327;
  double inv_std = 1.0 / stdev;
  double a = (x - mu) * inv_std;

  return inv_sqrt_2pi / inv_std * exp(-0.5f * a * a);
}
...
inline bool read_map_data(std::string filename, Map& map) {
	...
	while(getline(in_file_map, line_map)){
		...
		map.InsertLandmark(single_landmark_temp);
	}
	return true;
}
